...
export = function (Posts: PostsMethods) {
  ...
  Posts.getPostSummaryByPids = async function (pids, uid, options) {
    ...
    let posts = await Posts.getPostsFields(pids, fields as Array<keyof PostObject>)
    ...
    const result = await plugins.hooks.fire('filter:post.getPostSummaryByPids', { posts, uid }) as PostsWrapper
    return result.posts
  }
}
