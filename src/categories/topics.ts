...
export = function (Categories: CategoryMethods) {
  async function filterScheduledTids (tids: Array<number | string>): Promise<Array<number | string>> {
  ...
  Categories.getTopicIds = async function (data) {
    ...
    let normalTids: Array<number | string>
    const reverse = direction === 'highest-to-lowest'
    if (Array.isArray(set)) {
      const weights = set.map((s, index) => (index ? 0 : 1))
      // The next line calls a function in a module that has not been updated to TS yet
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
      normalTids = await db[reverse ? 'getSortedSetRevIntersect' : 'getSortedSetIntersect']({ sets: set, start, stop, weights }) as Array<string | number>
    } else {
      // The next line calls a function in a module that has not been updated to TS yet
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
      normalTids = await db[reverse ? 'getSortedSetRevRange' : 'getSortedSetRange'](set, start, stop) as Array<string | number>
    }
  ...
  Categories.getAllTopicIds = async function (cid, start, stop) {
    // The next line calls a function in a module that has not been updated to TS yet
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    return await db.getSortedSetRange([`cid:${cid}:tids:pinned`, `cid:${cid}:tids`], start, stop) as Array<string | number>
  }
  ...
  Categories.getPinnedTids = async function (data) {
    ...
    if (plugins.hooks.hasListeners('filter:categories.getPinnedTids')) {
      ...
      const result = await plugins.hooks.fire('filter:categories.getPinnedTids', {
        ...
        pinnedTids: Array<string | number>
      }
    ...
    const [allPinnedTids, canSchedule] = await Promise.all([
      // The next line calls a function in a module that has not been updated to TS yet
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
      db.getSortedSetRevRange(`cid:${data.cid}:tids:pinned`, data.start, data.stop) as Array<number | string>,
      // The next line calls a function in a module that has not been updated to TS yet
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    ...
    // The next line calls a function in a module that has not been updated to TS yet
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    return await topics.tools.checkPinExpiry(pinnedTids) as Array<string | number>
  }
