...
async function filterTids(privilege, tids, uid) {
  ...
  tids = topicsData.filter(t => cidsSet.has(t.cid) && (results.isAdmin || canViewDeletedScheduled(t, {}, canViewDeleted[t.cid], canViewScheduled[t.cid]))).map(t => t.tid);
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
...
async function canDelete(tid, uid) {
  ...
  if (!isModerator && preventTopicDeleteAfterReplies && topicData.postcount - 1 >= preventTopicDeleteAfterReplies) {
    ...
    // The next line calls a function in a module that has not been updated to TS yet
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    `[[error:cant-delete-topic-has-replies, ${meta_1.default.config.preventTopicDeleteAfterReplies}]]` : '[[error:cant-delete-topic-has-reply]]';
...
async function isOwnerOrAdminOrMod(tid, uid) {
  ...
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
  topics_1.default.isOwner(tid, uid), isAdminOrMod(tid, uid)]);
